...
<form method="GET" action="index.php" class="row g-2 align-items-end">
  ...
  <div class="col-12 col-md-4">
    ...
    <input type="text" name="buscar" class="form-control form-control-sm" placeholder="Codigo, marca, modelo, serie..." value="<?= htmlspecialchars($_GET['buscar'] ?? '') ?>">
  </div>
  <div class="col-6 col-md-3">
    <label class="form-label mb-1" style="font-size:.78rem">Categoria</label>
    <select name="categoria_id" class="form-select form-select-sm">
      <option value="">Todas</option>
      <?php foreach ($categorias as $cat): ?>
      <option value="<?= $cat['id'] ?>" <?= (($_GET['categoria_id'] ?? '') == $cat['id']) ? 'selected' : '' ?>><?= htmlspecialchars($cat['nombre']) ?></option>
      <?php endforeach; ?>
    </select>
  </div>
  <div class="col-6 col-md-3">
    ...
    <select name="estado" class="form-select form-select-sm">
      ...
      <option value="<?= $k ?>" <?= (($_GET['estado'] ?? '') === $k) ? 'selected' : '' ?>><?= $e['label'] ?></option>
      <?php endforeach; ?>
    </select>
  </div>
  <div class="col-12 col-md-2 d-flex gap-2">
    <button type="submit" class="btn btn-primary btn-sm w-100"><i class="bi bi-search me-1"></i>Filtrar</button>
    <a href="index.php?c=inventario" class="btn btn-outline-secondary btn-sm" title="Limpiar"><i class="bi bi-x-lg"></i></a>
  </div>
</form>
</div></div>
...
<div class="card">
  ...
  <div class="table-responsive"><table class="table mb-0 js-data-table">
    <thead><tr><th>Imagen</th><th>Codigo</th><th>N. Serie</th><th>Categoria</th><th>Marca / Modelo</th><th>Ubicacion</th><th>Asignacion activa</th><th>Estado</th><th style="width:80px"></th></tr></thead>
    <tbody>
    ...
    <tr style="cursor:pointer" onclick="location.href='index.php?c=inventario&a=detail&id=<?= $eq['id'] ?>'">
      <td><div style="width:44px;height:44px;border-radius:8px;overflow:hidden;background:#f0f4f9;display:grid;place-items:center">
        <?php if ($eq['img_principal']): ?><img src="index.php?c=inventario&a=imagen&img_id=<?= $eq['img_principal_id'] ?>&thumb=1" style="width:44px;height:44px;object-fit:cover" loading="lazy"><?php else: ?><i class="<?= $eq['categoria_icono'] ?> text-muted"></i><?php endif; ?>
      </div></td>
      <td><span class="folio-badge"><?= htmlspecialchars($eq['codigo']) ?></span></td>
      <td style="font-size:.84rem"><?= htmlspecialchars($eq['numero_serie'] ?: '-') ?></td>
      ...
      <td style="font-size:.84rem" onclick="event.stopPropagation()">
        <?php if (!empty($eq['asignacion_activa_id'])): ?>
        <a href="index.php?c=asignaciones&a=detail&id=<?= $eq['asignacion_activa_id'] ?>" class="text-decoration-none">
          <span class="folio-badge"><?= htmlspecialchars($eq['asignacion_activa_folio']) ?></span>
          <br><span style="color:#6b7c93;font-size:.78rem"><?= htmlspecialchars($eq['asignacion_activa_usuario']) ?></span>
        </a>
        <?php else: ?>
      ...
      <td onclick="event.stopPropagation()">
        <a href="index.php?c=inventario&a=detail&id=<?= $eq['id'] ?>" class="btn btn-sm btn-outline-primary me-1"><i class="bi bi-eye"></i></a>
        <a href="index.php?c=inventario&a=edit&id=<?= $eq['id'] ?>" class="btn btn-sm btn-outline-secondary"><i class="bi bi-pencil"></i></a>
      </td>
    </tr>
    <?php endforeach; ?>
    </tbody>
