...
?>
<?php if ($success): ?>
<div class="alert alert-success d-flex gap-2 mb-3"><i class="bi bi-check-circle-fill"></i>Operacion completada.</div>
<?php endif; ?>
...
<form method="GET" action="index.php" class="row g-2 align-items-end">
  ...
  <div class="col-12 col-md-5"><label class="form-label mb-1" style="font-size:.78rem">Buscar</label><input type="text" name="buscar" class="form-control form-control-sm" placeholder="Folio, usuario, codigo..." value="<?= htmlspecialchars($_GET['buscar'] ?? '') ?>"></div>
...
<div class="card">
  ...
  <div class="card-body text-center py-5 text-muted"><i class="bi bi-inbox" style="font-size:3rem"></i><p class="mt-3 mb-0">Sin asignaciones.</p></div>
  <?php else: ?>
  <div class="table-responsive"><table class="table mb-0 js-data-table">
    <thead><tr><th>Folio</th><th>Equipo</th><th>N. Serie</th><th>Asignado a</th><th>Entrega</th><th>Devolucion</th><th>Estado</th><th style="width:80px"></th></tr></thead>
    <tbody>
    ...
    <tr style="cursor:pointer;<?= $vencida ? 'background:#fff5f5' : '' ?>" onclick="location.href='index.php?c=asignaciones&a=detail&id=<?= $a['id'] ?>'">
      ...
      <td style="font-size:.84rem"><?= htmlspecialchars($a['equipo_serie'] ?: '-') ?></td>
